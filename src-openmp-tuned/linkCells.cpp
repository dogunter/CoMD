...
void updateLinkCells(LinkCell* boxes, Atoms* atoms)
{
   ...
   for (int iBox=0; iBox<boxes->nLocalBoxes; ++iBox)
   {
      ...
      {
         r[0] = atoms->r_0[iOff+ii];
         r[1] = atoms->r_1[iOff+ii];
         r[2] = atoms->r_2[iOff+ii];
         int jBox = getBoxFromCoord(boxes,r);
         if (jBox != iBox)
...
void copyAtom(LinkCell* boxes, Atoms* atoms, int iAtom, int iBox, int jAtom, int jBox)
{
   ...
   memcpy(atoms->r_0+jOff, atoms->r_0+iOff, sizeof(real_t));
   memcpy(atoms->r_1+jOff, atoms->r_1+iOff, sizeof(real_t));
   memcpy(atoms->r_2+jOff, atoms->r_2+iOff, sizeof(real_t));
   
   memcpy(atoms->p[jOff], atoms->p[iOff], sizeof(real3));

   memcpy(atoms->f_0+jOff, atoms->f_0+iOff, sizeof(real_t));
   memcpy(atoms->f_1+jOff, atoms->f_1+iOff, sizeof(real_t));
   memcpy(atoms->f_2+jOff, atoms->f_2+iOff, sizeof(real_t));

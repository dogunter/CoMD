...
Atoms* initAtoms(LinkCell* boxes)
{
   ...
   atoms->r_0 =        (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->r_1 =        (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->r_2 =        (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));

   atoms->f_0 =        (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->f_1 =        (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
   atoms->f_2 =        (real_t*) comdMalloc(maxTotalAtoms*sizeof(real_t));
...
void randomDisplacements(SimFlat* s, real_t delta)
{
   Kokkos::parallel_for(s->boxes->nLocalBoxes, KOKKOS_LAMBDA( const int& iBox)
   {
      for (int iOff=MAXATOMS*iBox, ii=0; ii<s->boxes->nAtoms[iBox]; ++ii, ++iOff)
      {
         ...
         s->atoms->r_0[iOff] += (2.0*lcg61(&seed)-1.0) * delta;
         s->atoms->r_1[iOff] += (2.0*lcg61(&seed)-1.0) * delta;
         s->atoms->r_2[iOff] += (2.0*lcg61(&seed)-1.0) * delta;
      }
   });
}
...
void computeVcm(SimFlat* s, real_t vcm[3])
{
   ...
   Kokkos::parallel_reduce(s->boxes->nLocalBoxes, KOKKOS_LAMBDA( const int& iBox, vstruct& local_v)
   {
      for (int iOff=MAXATOMS*iBox, ii=0; ii<s->boxes->nAtoms[iBox]; ++ii, ++iOff)
      {

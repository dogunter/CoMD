...
//      parms->cellList[ii] = mkAtomCellList(boxes, ii, parms->nCells[ii]);
   parms->cellList[0] = mkAtomCellList(boxes, HALO_X_MINUS, parms->nCells[0]);
   parms->cellList[1] = mkAtomCellList(boxes, HALO_X_PLUS, parms->nCells[1]);
   parms->cellList[2] = mkAtomCellList(boxes, HALO_Y_MINUS, parms->nCells[2]);
   parms->cellList[3] = mkAtomCellList(boxes, HALO_Y_PLUS, parms->nCells[3]);
   parms->cellList[4] = mkAtomCellList(boxes, HALO_Z_MINUS, parms->nCells[4]);
   parms->cellList[5] = mkAtomCellList(boxes, HALO_Z_PLUS, parms->nCells[5]);
...
{
   enum HaloFaceOrder faceM;
   enum HaloFaceOrder faceP;
   if ( iAxis == 0 ) {
      faceM = HALO_X_MINUS;
      faceP = HALO_X_PLUS;
   } else if ( iAxis == 1 ) {
...
int loadAtomsBuffer(void* vparms, void* data, int face, char* charBuf)
{
   ...
   for (int iCell=0; iCell<nCells; ++iCell)
   {
      ...
      for (int ii=iOff; ii<iOff+s->boxes->nAtoms[iBox]; ++ii)
      {
         ...
         buf[nBuf].rx = s->atoms->r_0[ii] + shift[0];
         buf[nBuf].ry = s->atoms->r_1[ii] + shift[1];
         buf[nBuf].rz = s->atoms->r_2[ii] + shift[2];
...
void sortAtomsInCell(Atoms* atoms, LinkCell* boxes, int iBox)
{
   ...
   for (int ii=begin, iTmp=0; ii<end; ++ii, ++iTmp)
   {
      ...
      tmp[iTmp].rx =   atoms->r_0[ii];
      tmp[iTmp].ry =   atoms->r_1[ii];
      tmp[iTmp].rz =   atoms->r_2[ii];
   ...
   for (int ii=begin, iTmp=0; ii<end; ++ii, ++iTmp)
   {
      ...
      atoms->r_0[ii]  = tmp[iTmp].rx;
      atoms->r_1[ii]  = tmp[iTmp].ry;
      atoms->r_2[ii]  = tmp[iTmp].rz;

...
//__attribute__((vector(uniform(table))))void interpolate(InterpolationObject* table, real_t r, real_t* f, real_t* df);
...
int eamForce(SimFlat* s)
{
   ...
   __assume_aligned(pot->dfEmbed,_ALIGN_INT);
   __assume_aligned(pot->rhobar,_ALIGN_INT);
#endif
   ...
   __assume_aligned(pot->phi,_ALIGN_INT);
   __assume_aligned(pot->rho,_ALIGN_INT);
   __assume_aligned(pot->f,_ALIGN_INT);
#endif
   InterpolationObject* table_Phi = pot->phi;
   InterpolationObject* table_Rho = pot->rho;
 
   int nNbrBoxes = 27;
   // loop over local boxes
   //#pragma omp parallel for reduction(+:etot)
   //#pragma omp parallel for default(shared) reduction(+:etot) 
   //for (int iBox=0; iBox<s->boxes->nLocalBoxes; iBox++)
   Kokkos::parallel_reduce( s->boxes->nLocalBoxes, KOKKOS_LAMBDA( const int& iBox, real_t& local_etot )
   {
      ...
      for (int jTmp=0; jTmp<nNbrBoxes; jTmp++)
      {
         ...
         for (int iOff=MAXATOMS*iBox; iOff<(iBox*MAXATOMS+nIBox); iOff++)
         {
            ...
            //#pragma simd reduction(+:sum_U,sum_Rho,sum_F_0,sum_F_1,sum_F_2,etot)
            for (int jOff=MAXATOMS*jBox; jOff<(jBox*MAXATOMS+nJBox); jOff++)
            {
               ...
               dr[0] = s->atoms->r_0[iOff] - s->atoms->r_0[jOff];
               r2+=dr[0]*dr[0];
               dr[1] = s->atoms->r_1[iOff] - s->atoms->r_1[jOff];
               r2+=dr[1]*dr[1];
               dr[2] = s->atoms->r_2[iOff] - s->atoms->r_2[jOff];
               r2+=dr[2]*dr[2];
               ...
               if(r2 <= rCut2 && r2 > 0.0)
               {
                  ...
                  interpolate(table_Phi, r, &phiTmp, &dPhi);
                  interpolate(table_Rho, r, &rhoTmp, &dRho);
                  
                  real_t dPhibyr = dPhi/r;  
                  sum_F_0 += dPhibyr*dr[0];
                  sum_F_1 += dPhibyr*dr[1];
                  sum_F_2 += dPhibyr*dr[2];

                  // Calculate energy contribution
                  sum_U += 0.5*phiTmp; 
                  local_etot  += 0.5*phiTmp;
   ...
   InterpolationObject* table_F = pot->f;
   ...
   Kokkos::parallel_reduce( s->boxes->nLocalBoxes, KOKKOS_LAMBDA( const int& iBox, real_t& local_etot)
   {
      ...
      for (int iOff=MAXATOMS*iBox; iOff<(MAXATOMS*iBox+nIBox); iOff++)
      {
         ...
         interpolate(table_F, pot->rhobar[iOff], &fEmbed, &dfEmbed);
   ...
   // third pass
   // loop over local boxes
   //#pragma omp parallel for //private(table_Rho)
   //for (int iBox=0; iBox<s->boxes->nLocalBoxes; iBox++)
   Kokkos::parallel_for( s->boxes->nLocalBoxes, KOKKOS_LAMBDA( const int& iBox )
   {
      int nIBox = s->boxes->nAtoms[iBox];
      ...
      for (int jTmp=0; jTmp<nNbrBoxes; jTmp++)
      {
         ...
         {
            // loop over atoms in jBox
            //#pragma omp simd reduction(+:sum_F_0,sum_F_1,sum_F_2)
            ...
            { 
               ...
               if(r2 <= rCut2 && r2 > 0.0)
               {
                  ...
                  real_t dRhobyr = dRho/r;
                  sum_F_0 += (pot->dfEmbed[iOff]+pot->dfEmbed[jOff])*dRhobyr*dr[0];
                  sum_F_1 += (pot->dfEmbed[iOff]+pot->dfEmbed[jOff])*dRhobyr*dr[1];
                  sum_F_2 += (pot->dfEmbed[iOff]+pot->dfEmbed[jOff])*dRhobyr*dr[2];
               }             
            } // loop over atoms in jBox
         s->atoms->f_0[iOff] -= sum_F_0;
         s->atoms->f_1[iOff] -= sum_F_1;
   ...
   int n, real_t x0, real_t dx, real_t* data)
{
   ...
   table->values = (real_t*)comdMalloc((n+3)*sizeof(real_t));
   assert(table->values);

   for(int ii=0; ii<n+3; ii++)table->values[ii] = 1.0;
...
#ifdef ASSUME_ALIGN
   __assume_aligned(table->values,_ALIGN_INT);
#endif
...
//try__1: __attribute__((vector(uniform(table))))
#pragma omp declare simd aligned(table:_ALIGN_INT) inbranch
void interpolate(InterpolationObject* table, real_t r, real_t* f, real_t* df)
{
   ...
   r = r - floor(r);
   real_t tt_[4] = {table->values[ii-1],table->values[ii],table->values[ii+1],table->values[ii+2]};
   
   real_t g1 = tt_[2] - tt_[0];
   real_t g2 = tt_[3] - tt_[1];

   *f = tt_[1] + 0.5*r*(g1 + r*(tt_[2] + tt_[0] - 2.0*tt_[1]) );
...
#ifdef ASSUME_ALIGN
   __assume_aligned(buf,_ALIGN_INT);
#endif

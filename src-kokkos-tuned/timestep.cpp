...
void advanceVelocity(SimFlat* s, int nBoxes, real_t dt)
{
   ...
   Kokkos::parallel_for( nBoxes, KOKKOS_LAMBDA( const int& iBox )
   {
      for (int iOff=MAXATOMS*iBox,ii=0; ii<s->boxes->nAtoms[iBox]; ii++,iOff++)
      {
         s->atoms->p[iOff][0] += dt*s->atoms->f_0[iOff];
         s->atoms->p[iOff][1] += dt*s->atoms->f_1[iOff];
         s->atoms->p[iOff][2] += dt*s->atoms->f_2[iOff];
      }
   });
}
...
void advancePosition(SimFlat* s, int nBoxes, real_t dt)
{
   ...
   Kokkos::parallel_for( nBoxes, KOKKOS_LAMBDA( const int& iBox )
   {
      for (int iOff=MAXATOMS*iBox,ii=0; ii<s->boxes->nAtoms[iBox]; ii++,iOff++)
      {
         ...
         s->atoms->r_0[iOff] += dt*s->atoms->p[iOff][0]*invMass;
         s->atoms->r_1[iOff] += dt*s->atoms->p[iOff][1]*invMass;
         s->atoms->r_2[iOff] += dt*s->atoms->p[iOff][2]*invMass;
      }
   });
}
...
void kineticEnergy(SimFlat* s)
{
   ...
   Kokkos::parallel_reduce( s->boxes->nLocalBoxes, KOKKOS_LAMBDA( const int& iBox, real_t& local_kenergy )
   {
      for (int iOff=MAXATOMS*iBox,ii=0; ii<s->boxes->nAtoms[iBox]; ii++,iOff++)
      {
         int iSpecies = s->atoms->iSpecies[iOff];
         real_t invMass = 0.5/s->species[iSpecies].mass;
         local_kenergy += ( s->atoms->p[iOff][0] * s->atoms->p[iOff][0] +
